...
def add_mid_phantom(graph: Graph, start_vertex: str, end_vertex: str, floor_id: int):
    ...
    mid_vertex = f"mid_{start_vertex}_{end_vertex}"
    graph.add_vertex(mid_vertex, (mid_x, mid_y, floor_id))
    dist_start = sqrt((mid_x - start_coords[0]) ** 2 + (mid_y - start_coords[1]) ** 2)
    dist_end = sqrt((mid_x - end_coords[0]) ** 2 + (mid_y - end_coords[1]) ** 2)
    graph.add_edge(start_vertex, mid_vertex, dist_start)
    graph.add_edge(mid_vertex, end_vertex, dist_end)
    graph.add_edge(mid_vertex, start_vertex, dist_start)
    graph.add_edge(end_vertex, mid_vertex, dist_end)
    logger.info(f"Added mid phantom vertex: {mid_vertex} -> ({mid_x}, {mid_y}, {floor_id})")
...
def add_vertex_to_graph(graph: Graph, db: Session, vertex: str):
    ...
    if type_ == "room":
        ...
        add_mid_phantom(graph, f"segment_{id_}_start", f"segment_{id_}_end", segment.floor_id)
        ...
        add_mid_phantom(graph, f"outdoor_{id_}_start", f"outdoor_{id_}_end", 1)
...
def get_relevant_floors(db: Session, start: str, end: str) -> set:
    ...
    floor_ids.add(1)  # Для уличных сегментов
...
def build_graph(db: Session, start: str, end: str) -> Graph:
    ...
    for segment in segments:
        ...
        if start_vertex not in graph.vertices:
            ...
            add_mid_phantom(graph, start_vertex, end_vertex, segment.floor_id)
    ...
    for outdoor in outdoor_segments:
        ...
        if start_vertex not in graph.vertices:
            ...
            add_mid_phantom(graph, start_vertex, end_vertex, 1)
            ...
            if from_segment and to_segment:
                ...
                if from_vertex in graph.vertices and to_vertex in graph.vertices:
                    if (from_vertex, to_vertex) not in [(e[0], e[1]) for e in graph.edges.get(from_vertex, [])]:
                        ...
                        additional_weight = 0 if from_floor_id == to_floor_id else 10  # Штраф только для разных этажей
                        ...
                        logger.info(f"Added edge ({conn.type}): {from_vertex} <-> {to_vertex}, weight={weight + additional_weight}, from_floor={from_floor_id}, to_floor={to_floor_id}")
