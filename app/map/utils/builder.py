...
def align_coordinates(from_coords: tuple, to_coords: tuple, from_seg_start: tuple, from_seg_end: tuple, to_seg_start: tuple, to_seg_end: tuple) -> tuple:
    """Выравнивает координаты для переходов, сохраняя реальные значения."""
    from_x, from_y, from_z = from_coords
    to_x, to_y, to_z = to_coords

    # Используем реальные координаты из сегментов без принудительного выравнивания
    dist_x = abs(from_x - to_x)
    dist_y = abs(from_y - to_y)
    if dist_x < dist_y:
        # Если больше движение по Y, сохраняем X
        aligned_x = from_seg_end[0]
        return (aligned_x, from_y, from_z), (aligned_x, to_y, to_z)
    else:
        # Если больше движение по X, сохраняем Y
...
def build_graph(rooms: dict, segments: dict, outdoor_segments: dict, connections: dict) -> Graph:
    ...
    # Добавляем сегменты и их фантомные точки
    for seg_id, (start_vertex, end_vertex) in segments.items():
        ...
        # Добавляем фантомные точки для лестниц и переходов
    ...
    # Обрабатываем связи из таблицы connections
    for conn_id, conn in connections.items():
        # Комната-сегмент
        if conn.from_room_id and conn.to_segment_id and conn.from_room_id in rooms and conn.to_segment_id in segments:
