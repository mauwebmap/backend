    ...
    for conn in db.query(Connection).all():
        ...
        if conn.from_segment_id and conn.to_segment_id:
            ...
            if from_segment and to_segment:
                # Определяем Y для лестницы из начального сегмента
                if from_segment.start_y == from_segment.end_y:  # Горизонтальный сегмент
                    y = from_segment.start_y
                else:  # Вертикальный сегмент
                    y = (from_segment.start_y + from_segment.end_y) / 2  # Среднее Y

                # Координаты фантомных точек на обоих этажах
                ...
                # Для from_floor (начальный этаж)
                from_x = (from_segment.start_x + from_segment.end_x) / 2  # Среднее X для плавности
                from_coords = (from_x, y, from_floor)
                graph.add_vertex(phantom_from, {"coords": from_coords, "building_id": None})

                # Для to_floor (конечный этаж)
                to_x = (to_segment.start_x + to_segment.end_x) / 2  # Среднее X для плавности
                to_coords = (to_x, y, to_floor)  # Сохраняем тот же Y
