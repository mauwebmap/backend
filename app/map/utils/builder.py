...
def build_graph(db: Session, start: str, end: str) -> Graph:
    ...
    for conn in db.query(Connection).all():
        ...
        if conn.from_segment_id and conn.to_segment_id:
            ...
            # Координаты начала from_segment
            from_coords = graph.get_vertex_data(from_start)["coords"]
            to_coords = (from_coords[0], from_coords[1], to_floor)  # Сохраняем Y, меняем этаж
            # Фантомная точка перед лестницей на том же Y
            phantom_pre_stair = f"phantom_pre_stair_{conn.from_segment_id}_to_{conn.to_segment_id}"
            # Берем X как среднее между from_start и from_end для естественного положения
            from_start_coords = graph.get_vertex_data(from_start)["coords"]
            from_end_coords = graph.get_vertex_data(from_end)["coords"]
            pre_stair_x = (from_start_coords[0] + from_end_coords[0]) / 2
            pre_stair_coords = (pre_stair_x, from_coords[1], from_floor)
            graph.add_vertex(phantom_pre_stair, {"coords": pre_stair_coords, "building_id": None})
            # Основные точки лестницы
            ...
            far_coords_from = (from_coords[0] + 10, from_coords[1], from_floor)
            ...
            # Соединяем точки
            weight = conn.weight if conn.weight else 2.0
            graph.add_edge(from_start, phantom_pre_stair, weight, {"type": "segment"})
            graph.add_edge(from_end, phantom_pre_stair, weight, {"type": "segment"})
            graph.add_edge(phantom_pre_stair, phantom_from, weight, {"type": "segment"})
