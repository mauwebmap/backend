...
async def get_svg(campus_id: int, db: Session = Depends(get_db)):
    campus = get_campus(db, campus_id)
    if not campus or not campus.image_path:
        return {"error": "SVG not found"}

    return FileResponse(campus.image_path.lstrip('/'))

@router.post("/", response_model=Campus)
async def create_campus_endpoint(
    name: str = Form(...),
    description: str = Form(...),
    svg_file: UploadFile = File(None),
    db: Session = Depends(get_db)
):
    return await create_campus(
        db=db,
        name=name,
